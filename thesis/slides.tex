...
\begin{frame}{Preliminaries | Selfish routing (1\textsuperscript{st} level)}
	...
	since $\ell_1\left(\frac15\right) + 3 = \ell_2\left(\frac25\right) + 2 = \ell_3\left(\frac25\right) + 1 = \frac{17}{5}$.
\end{frame}
...
\begin{frame}{Sensitivity split | Heterogeneous networks}
	Latencies are the same for all players.\pause

	Toll ordering is the same for all players.

	Toll differences are viewed with different scaling.\pause[1]
	\\[15pt]
	\begin{tikzpicture}
		% Axes
		\draw[->] (0,0) -- (7,0) node[right=5,fixed_height] {$c$};
		\draw[-] (0,0) -- (0,3.2);
		...
		\onslide<2->{
			...
			% Y-axis label
			\node[right=12,fixed_height] at (7,0) {$(0)$};
...
\begin{frame}{Sensitivity split | Example}
	\begin{center}
		\begin{multicols}{2}
			% Left Column: Network Graph
			\begin{tikzpicture}[scale=1.2, baseline=40pt, every node/.style={font=\small}]
		...
		\end{multicols}
	\end{center}
	\[x(0) = \left(\frac23, \frac13\right) \qquad \as(t) = \alpha(\text{lower split flow})\]
\end{frame}
...
\begin{frame}{Sensitivity split | Example}
	\begin{figure}
		...
		\end{tikzpicture}
		\caption{Flow function of link $1$ of Example}
...
\begin{frame}{Sensitivity split | Upper bound}
	...
	\[\min\{x_1(0), x_2(0)\} = \max\left\{\tfrac23, \tfrac13\right\} = \tfrac13\]\pause
	...
	\[
		...
		\end{cases}
	\]
	then $\as(t)$ would remain the same for all valid $t \in \R_+^n$.
\end{frame}
...
\begin{frame}{Pseudo-heterogeneous Pricing Games}
	Consider distribution functions $\alpha(p) = c$ for some $c > 0$.\pause

	Players again all view link costs the same.
	\[c_i(p) = \ell_i(x_i) + c t_i\]
	Behaviour should be homogeneous with tolls "mapped" to $t \rightarrow t/c$.\pause
...
\begin{frame}{Pseudo-heterogeneous Pricing Games}
	We will call such games \textit{pseudo-heterogeneous}.\pause

	Uniform increase in money-sensitivity is equivalent to uniform decrease in latencies (lost for general $\alpha$ functions).\pause
...
\begin{frame}{Step distribution functions}
	\begin{figure}
		...
		\begin{tikzpicture}[x=3cm, y=3cm, every node/.style={font=\small}]
			...
			% Tick marks
			\draw (-1,0) -- (-1,-0.05) node[below] {$-1$};
			\draw (-2/5,0) -- (-2/5,-0.05) node[left=4,below] {$-\frac25$};
			\draw (-2/15,0) -- (-2/15,-0.05) node[left=4,below] {$-\frac{2}{15}$};
			\draw (0,0) -- (0,-0.05) node[below] {$t_2$};
			\draw (1/8,0) -- (1/8,-0.05) node[left=1,below] {$\frac18$};
			\draw (1/6,0) -- (1/6,-0.05) node[right=2,below] {$\frac16$};
...
\begin{frame}{Step distribution functions}
	\begin{figure}
		...
		\begin{tikzpicture}[x=3.4cm, y=1cm, scale=0.8, every node/.style={font=\small}]
			...
			\draw (-2/5,0) -- (-2/5,-0.2) node[left=4,below] {$-\frac25$};
			\draw (-2/15,0) -- (-2/15,-0.2) node[left=4,below] {$-\frac{2}{15}$};
			\draw (0,0) -- (0,-0.2) node[below] {$t_2$};
			\draw (1/8,0) -- (1/8,-0.2) node[left=1,below] {$\frac18$};
			\draw (1/6,0) -- (1/6,-0.2) node[right=2,below] {$\frac16$};
			\draw (7/15,0) -- (7/15,-0.2) node[below] {$\frac{7}{15}$};
			\draw (7/5,0) -- (7/5,-0.2) node[below] {$\frac75$};
			\draw (2,0) -- (2,-0.2) node[below] {$2$};
			\draw (0,1) -- (-0.08,1) node[left=1] {$1$};
			\draw (0,3) node[right=1] {$3$} -- (0.05,3);
			\draw (0,4) -- (-0.08,4) node[left=1] {$4$};
...
\begin{frame}{Step distribution functions}
	...
	\end{lemma}
	\begin{theorem}
		If $\Gm_1$ has $t^*$, let $\Gm_2 = ([2], \ell, \alpha^{(2)})$ with fixed $\alpha^{(2)}(p) = \as(t^*)$.
	...
	\begin{center}
		\begin{multicols}{2}
			...
			\begin{tikzpicture}[x=3cm, y=0.7cm, every node/.style={font=\small}]
				...
				% Tick marks
				\draw (0,0) -- (0,-0.05) node[below] {$0$};
				\draw (0.25,0) -- (0.25,-0.05) node[below] {$\frac14$};
				\draw (1,0) -- (1,-0.05) node[below] {$1$};
				\draw (0,3) -- (-0.05,3) node[left] {$3$};
				\draw (0,4) -- (-0.05,4) node[left] {$4$};

				% Function graph
				\draw[thick] (0,3) -- (0.25,3);
				\draw[thick] (0.25,4) -- (1,4);

				% Closed / Open dots
				\fill[black] (0.25,3) ellipse (0.014 and 0.06);
				\draw[black] (0.25,3) ellipse (0.014 and 0.06);
				\fill[white] (0.25,4) ellipse (0.014 and 0.06);
				\draw[black] (0.25,4) ellipse (0.014 and 0.06);
			\end{tikzpicture}
		\end{multicols}
	\end{center}
\end{frame}
...
\begin{frame}{Step distribution functions}
	If $t^*$ exists, it needs to be at $\as(t^*) = 3$ or $4$ (not between).\pause

	Flow will be the same regardless.\pause

	Assume $\Gm^\prime = (N, \ell, 1)$ and we calculate
	\[t^{*\prime} = \left(\frac53, \frac43\right) \qquad x^\prime(t^{*\prime}) = \left(\frac59, \frac49\right)\]\pause

	Since $t_1 > t_2$, link $1$ has the lower split, so the split is at $\alpha\left(\frac59\right) = 4$.\pause
...
\begin{frame}{Step distribution functions}
	\begin{figure}
		...
		\begin{tikzpicture}[x=8cm, y=10cm, scale=0.7, every node/.style={font=\small}]
			...
			\draw[->] (0,0) -- (0,0.3) node[above] {$\Pi_1\left(t_1, \frac{4}{12}\right)$};

			% Tick marks
			\draw (-1/3,0) -- (-1/3,-0.015) node[left=4,below] {$-\frac13$};
			\draw (-1/4,0) -- (-1/4,-0.015) node[left=4,below] {$-\frac14$};
			\draw (-1/12,0) -- (-1/12,-0.015) node[left=9,below] {$-\frac{1}{12}$};
			\draw (-1/16,0) -- (-1/16,-0.015) node[right=4,below] {$-\frac{1}{16}$};
			%\draw (0,0) -- (0,-0.015) node[below] {$0$};
			\draw (1/12,0) -- (1/12,-0.015) node[below] {$\frac{1}{12}$};
			\draw (5/16,0) -- (5/16,-0.015) node[below] {$\frac{5}{16}$};
			\draw (5/12,0) -- (5/12,-0.015) node[below] {$\frac{5}{12}$};
			\draw (2/4,0) -- (2/4,-0.015) node[below] {$\frac24$};
			\draw (2/3,0) -- (2/3,-0.015) node[below] {$\frac23$};

			% Pseudo-heterogeneous profits
			\draw[gray,dashed,thick,domain=-1/3:2/3] plot (\x,{((2-3*\x)/3)*(1/3+\x)}) node[right=3,above=8,black] {$c = 3$};
			\draw[gray,dashed,thick,domain=-1/4:2/4] plot (\x,{((2-4*\x)/3)*(1/3+\x)}) node[left=23,above=1,black] {$c = 4$};

			% Profit function
			\draw[thick,domain=-1/3:-1/12] plot (\x,{((2-3*\x)/3)*(4/12+\x)});
			\draw[thick] (-1/12,3/16) -- (-1/16, 39/192);
			\fill[black] (-1/12,3/16) ellipse (0.00625 and 0.005) node[right=2,below=-1] {$\frac{3}{16}$};
			\fill[black] (-1/16,39/192) ellipse (0.00625 and 0.005) node[left=7,above=-1] {$\frac{39}{192}$};
			\draw[thick,domain=-1/16:5/16] plot (\x,{((2-4*\x)/3)*(4/12+\x)});
			\draw[thick] (5/16,31/192) -- (5/12,3/16);
			\fill[black] (5/16,31/192) ellipse (0.00625 and 0.005) node[left=7,below=-3] {$\frac{31}{192}$};
			\fill[black] (5/12,3/16) ellipse (0.00625 and 0.005) node[right=7,above=-1] {$\frac{3}{16}$};
		...
		\begin{tikzpicture}[x=10cm, y=15cm, scale=0.7, every node/.style={font=\small}]
			...
			\draw[->] (0,0) -- (0,0.215) node[above] {$\Pi_2\left(\frac{5}{12}, t_2\right)$};
			...
			% Pseudo-heterogeneous profits
			\draw[gray,dashed,thick,domain=-5/12:1/3] plot (\x,{((1-3*\x)/3)*(5/12+\x)}) node[right=3,above=8,black] {$c = 3$};
			\draw[gray,dashed,thick,domain=-5/12:1/4] plot (\x,{((1-4*\x)/3)*(5/12+\x)}) node[left=23,above=1,black] {$c = 4$};

			% Profit function
			\draw[thick] (-5/12,0) -- (-5/16,5/64);
			\fill[black] (-5/12,0) ellipse (0.0045 and 0.003) node[left=4,above=-1] {$0$};
			\fill[black] (-5/16,5/64) ellipse (0.0045 and 0.003) node[left=4,above=-1] {$\frac{5}{64}$};
			\draw[thick,domain=-5/16:1/16] plot (\x,{((1-4*\x)/3)*(5/12+\x)});
			\draw[thick] (1/16,23/192) -- (1/12,1/8);
			\fill[black] (1/16,23/192) ellipse (0.0045 and 0.003) node[left=4,below=-1] {$\frac{23}{192}$};
			\fill[black] (1/12,1/8) ellipse (0.0045 and 0.003) node[right=4,above=-1] {$\frac18$};
	...
	\begin{center}
		\begin{multicols}{2}
			...
			\begin{tikzpicture}[x=3cm, y=0.7cm, every node/.style={font=\small}]
				...
				% Function graph
				\draw[thick] (0,2) -- (0.25,2);
				\draw[thick] (0.25,4) -- (1,4);

				% Closed / Open dots
				\fill[black] (0.25,2) ellipse (0.014 and 0.06);
				\draw[black] (0.25,2) ellipse (0.014 and 0.06);
				\fill[white] (0.25,4) ellipse (0.014 and 0.06);
				\draw[black] (0.25,4) ellipse (0.014 and 0.06);
			\end{tikzpicture}
		\end{multicols}
...
\begin{frame}{Step distribution functions}
	\begin{figure}
		...
		\begin{tikzpicture}[x=6cm, y=10cm, scale=0.7, every node/.style={font=\small}]
			...
			% Tick marks
			\draw (-1/3,0) -- (-1/3,-0.015) node[left=11,below] {$-\frac13$};
			\draw (-1/4,0) -- (-1/4,-0.015) node[left=8,below] {$-\frac14$};
			\draw (-1/8,0) -- (-1/8,-0.015) node[left=8,below] {$-\frac18$};
			\draw (-1/16,0) -- (-1/16,-0.015) node[right=1,below] {$-\frac{1}{16}$};
			%\draw (0,0) -- (0,-0.015) node[below] {$0$};
			\draw (1/12,0) -- (1/12,-0.015) node[below] {$\frac{1}{12}$};
			\draw (5/16,0) -- (5/16,-0.015) node[below] {$\frac{5}{16}$};
			\draw (2/4,0) -- (2/4,-0.015) node[below] {$\frac24$};
			\draw (5/8,0) -- (5/8,-0.015) node[below] {$\frac58$};
			\draw (1,0) -- (1,-0.015) node[below] {$1$};

			% Pseudo-heterogeneous profits
			\draw[gray,dashed,thick,domain=-1/3:2/2] plot (\x,{((2-2*\x)/3)*(1/3+\x)}) node[right=5,above=10,black] {$c = 2$};
			\draw[gray,dashed,thick,domain=-1/4:2/4] plot (\x,{((2-4*\x)/3)*(1/3+\x)}) node[right=10,above=5,black] {$c = 4$};

			% Profit function
			\draw[thick,domain=-1/3:-1/8] plot (\x,{((2-2*\x)/3)*(4/12+\x)});
			\draw[thick] (-1/8, 5/32) -- (-1/16, 39/192);
			\fill[black] (-1/8, 5/32) ellipse (0.008333 and 0.005) node[right=2,below=-1] {$\frac{5}{32}$};
			\fill[black] (-1/16,39/192) ellipse (0.008333 and 0.005) node[left=7,above=-1] {$\frac{39}{192}$};
			\draw[thick,domain=-1/16:5/16] plot (\x,{((2-4*\x)/3)*(4/12+\x)});
			\draw[thick] (5/16,31/192) -- (5/8,23/96);
			\fill[black] (5/16,31/192) ellipse (0.008333 and 0.005) node[left=7,below=-1] {$\frac{31}{192}$};
			\fill[black] (5/8,23/96) ellipse (0.008333 and 0.005) node[right=7,above=-1] {$\frac{23}{96}$};
		...
		\begin{tikzpicture}[x=8cm, y=15cm, scale=0.7, every node/.style={font=\small}]
			...
			% Tick marks
			%\draw (-2/4,0) -- (-2/4,-0.01) node[below] {$-\frac24$};
			\draw (-5/12,0) -- (-5/12,-0.01) node[left=3,below] {$-\frac{5}{12}$};
			\draw (-5/16,0) -- (-5/16,-0.01) node[below] {$-\frac{5}{16}$};
			\draw (-1/12,0) -- (-1/12,-0.01) node[left=5,below] {$-\frac{1}{12}$};
			\draw (0,0) -- (0,-0.01) node[below] {$0$};
			\draw (1/16,0) -- (1/16,-0.01) node[left=2,below] {$\frac{1}{16}$};
			\draw (1/12,0) -- (1/12,-0.01) node[right=5,below] {$\frac{1}{12}$};
			\draw (1/4,0) -- (1/4,-0.01) node[below] {$\frac14$};
			\draw (1/2,0) -- (1/2,-0.01) node[below] {$\frac12$};

			% Pseudo-heterogeneous profits
			\draw[gray,dashed,thick,domain=-5/12:1/2] plot (\x,{((1-2*\x)/3)*(5/12+\x)}) node[right=4,above=8,black] {$c = 2$};
			\draw[gray,dashed,thick,domain=-5/12:1/4] plot (\x,{((1-4*\x)/3)*(5/12+\x)}) node[right=8,above=4,black] {$c = 4$};

			% Profit function
			\draw[thick] (-5/12,0) -- (-5/16,5/64);
			\fill[black] (-5/16,5/64) ellipse (0.008 and 0.004) node[left=4,above=-1] {$\frac{5}{64}$};
			\draw[thick,domain=-5/16:1/16] plot (\x,{((1-4*\x)/3)*(5/12+\x)});
			\draw[thick] (1/16,23/192) -- (1/8,13/96);
			\fill[black] (1/16,23/192) ellipse (0.008 and 0.004) node[left=4,below=-1] {$\frac{23}{192}$};
			\fill[black] (1/8,13/96) ellipse (0.008 and 0.004) node[right=7,above=-1] {$\frac{13}{96}$};
...
\begin{frame}{Step distribution functions | Pricing N.E. Algorithm}
	\textbf{Summary:} Given $\Gm = ([2], \ell, \alpha), \alpha = (\alpha_k)_{k \in [m]}$\pause
	\begin{enumerate}
		\item Consider $\Gm^\prime = ([2], \ell, 1)$ and calculate $t^{*\prime}, x^\prime(t^{*\prime})$.\pause
		\item Calculate $\as(t^*)$ as the $\alpha$ value of the lower split in $x^\prime(t^{*\prime})$.\pause
		\item Calculate the candidate $t^* = \tfrac{t^{*\prime}}{\as(t^*)}$ and $\Pi_1(t^*), \Pi_2(t^*)$.\pause
		\item For each $\alpha_k \ne \as(t^*)$, and for each $i \in [2]$:\pause
		\begin{enumerate}[$4.1$]
			\item Consider $\Gm_k = ([2], \ell, \alpha_k)$ and calculate $B_{i, k}(t_{-i}^*)$ and tolls $t_{i, k}$ for which $\as(t_i, t_{-i}^*) = \alpha_k$, resulting in two toll ranges for $t_i$.\pause
			\item Search for $\Pi_{i, k}$, the max profit for $i$ within those $t_i$ ranges:\pause
			\begin{enumerate}[$4.2.1$]
				\item If $B_{i, k}(t_{-i}^*)$ is within the $t_i$ ranges, $\Pi_{i, k} = \Pi_i(B_{i, k}(t_{-i}^*), t_{-i}^*)$.\pause
