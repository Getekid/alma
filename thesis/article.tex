...
\begin{proof}
	First we verify that $t^* = \left(\tfrac59, \tfrac49\right)$ is indeed a valid candidate pricing Nash equilibrium.
	Theorem \ref{theorem:a_step_ne_equiv_fixed} requires $t^*$ to also be a Nash Equilibrium at the pricing game of $\Gm^c = ([2], \ell, c)$, a pseudo-heterogeneous game where $c = 2$ or $3$.
	...
	Such a flow split puts the flow of link $1$ in the upper split and link $2$ in the lower split, making $\as(t^*) = \alpha(x_1(t^{*\prime})) = \alpha(5/9) = 4$.
	...
	We can then take the profit definition, formula \ref{eq:homogeneous_x_i} for $2$ links as well as Lemma \ref{lemma:a_fixed_c_relation}$(ii)$ and get for $\Gm^c$
	\[
		\Pi_1^c(t_1, t_2^*) = t_1 \cdot \frac{2 + c (4/3 - t_1)}{3}
		\qquad
		\Pi_2^c(t_1^*, t_2) = t_2 \cdot \frac{1 + c (5/3 - t_2)}{3}
	\]
	...
	For toll differences where $\as(t) = c$, then profit is equal to $\Pi_i^c(t)$ (Lemma \ref{lemma:heterogenous_flow_equiv_fixed}).
	Otherwise, the flow remains static, making $x_i t_i$ a strictly increasing linear function with regards to $t_i$.
...
\begin{figure}
	\centering
	\begin{tikzpicture}[x=10cm, y=17cm, every node/.style={font=\small}]
		% Axes
		\draw[->] (0,0) -- (-0.4,0);
		...
		\draw (0,0) -- (0,-0.01) node[below] {$0$} node[below=15] {$t_2^* = \frac{4}{12}$};
		...
		\draw[gray,dashed,very thin,domain=-1/3:2/3] plot (\x,{((2-3*\x)/3)*(1/3+\x)}) node[above=20,right=-5,black] {$c = 3$};
		\draw[gray,dashed,very thin,domain=-1/4:2/4] plot (\x,{((2-4*\x)/3)*(1/3+\x)}) node[above=15,right,black] {$c = 4$};

		% Profit function
		\draw[thick,domain=-1/3:-1/12] plot (\x,{((2-3*\x)/3)*(4/12+\x)});
		\draw[thick] (-1/12,3/16) -- (-1/16, 39/192);
		\fill[black] (-1/12,3/16) ellipse (0.0051 and 0.003) node[below right] {$\frac{3}{16}$};
		\fill[black] (-1/16,39/192) ellipse (0.0051 and 0.003) node[above left] {$\frac{39}{192}$};
		\draw[thick,domain=-1/16:5/16] plot (\x,{((2-4*\x)/3)*(4/12+\x)});
		\draw[thick] (5/16,31/192) -- (5/12,3/16);
		\fill[black] (5/16,31/192) ellipse (0.0051 and 0.003) node[below left] {$\frac{31}{192}$};
		\fill[black] (5/12,3/16) ellipse (0.0051 and 0.003) node[above right] {$\frac{3}{16}$};
		\draw[thick,domain=5/12:2/3] plot (\x,{((2-3*\x)/3)*(4/12+\x)});

		% Max point
		\fill[red] (1/12, 25/108) ellipse (0.0068 and 0.004) node[below,black] {$\frac{25}{108}$};
	\end{tikzpicture}
	\begin{tikzpicture}[x=10cm, y=20cm, every node/.style={font=\small}]
		% Axes
		\draw[->] (0,0) -- (-0.55,0);
		...
		\draw (-5/16,0) -- (-5/16,-0.01) node[below] {$-\frac{5}{16}$};
		\draw (-1/12,0) -- (-1/12,-0.01) node[below] {$-\frac{1}{12}$};
		\draw (0,0) -- (0,-0.01) node[below] {$0$} node[below=15] {$t_1^* = \frac{5}{12}$};
		...
		% Pseudo-heterogeneous profits
		\draw[gray,dashed,very thin,domain=-5/12:1/3] plot (\x,{((1-3*\x)/3)*(5/12+\x)}) node[above=20,right=-5,black] {$c = 3$};
		\draw[gray,dashed,very thin,domain=-5/12:1/4] plot (\x,{((1-4*\x)/3)*(5/12+\x)}) node[above=15,left=17,black] {$c = 4$};

		% Profit function
		\draw[thick] (-5/12,0) -- (-5/16,5/64);
		\fill[black] (-5/12,0) ellipse (0.005 and 0.0025) node[above left] {$0$};
		\fill[black] (-5/16,5/64) ellipse (0.005 and 0.0025) node[above left] {$\frac{5}{64}$};
		\draw[thick,domain=-5/16:1/16] plot (\x,{((1-4*\x)/3)*(5/12+\x)});
		\draw[thick] (1/16,23/192) -- (1/12,1/8);
		\fill[black] (1/16,23/192) ellipse (0.005 and 0.0025) node[below left] {$\frac{23}{192}$};
		\fill[black] (1/12,1/8) ellipse (0.005 and 0.0025) node[above right] {$\frac18$};
		\draw[thick,domain=1/12:1/3] plot (\x,{((1-3*\x)/3)*(5/12+\x)});

		% Max point
		\fill[red] (-1/12, 4/27) ellipse (0.008 and 0.004) node[above,black] {$\frac{4}{27}$};
	\end{tikzpicture}
	\caption{Profit functions for link operators $1$ and $2$ of Example \ref{example:a_step_profit_ne}.\\Max profit for both at $t^* = \left(\tfrac{5}{12}, \tfrac{4}{12}\right)$.}
...
\begin{proof}
	...
	We calculate again the profit functions giving us Figure \ref{figure:alpha_step:profit_no_ne}, where we can see that
	\[
		B_1\left(\frac{4}{12}\right) = \frac58 \ne \frac{5}{12} \qquad B_2\left(\frac{5}{12}\right) = \frac{4}{12}
	\]
	Since $t^*$ is the only possible candidate (Theorem \ref{theorem:a_step_ne_equiv_fixed}), then $\Gm$ has no Nash equilibrium for its pricing game.
\end{proof}

\begin{figure}
	\centering
	\begin{tikzpicture}[x=6cm, y=17cm, every node/.style={font=\small}]
		% Axes
		\draw[->] (0,0) -- (-0.4,0);
		...
		% Pseudo-heterogeneous profits
		\draw[gray,dashed,very thin,domain=-1/3:2/2] plot (\x,{((2-2*\x)/3)*(1/3+\x)}) node[above=20,right=-5,black] {$c = 2$};
		\draw[gray,dashed,very thin,domain=-1/4:2/4] plot (\x,{((2-4*\x)/3)*(1/3+\x)}) node[above=15,right,black] {$c = 4$};

		% Profit function
		\draw[thick,domain=-1/3:-1/8] plot (\x,{((2-2*\x)/3)*(4/12+\x)});
		\draw[thick] (-1/8, 5/32) -- (-1/16, 39/192);
		\fill[black] (-1/8, 5/32) ellipse (0.0085 and 0.003) node[below right] {$\frac{5}{32}$};
		\fill[black] (-1/16,39/192) ellipse (0.0085 and 0.003) node[above left] {$\frac{39}{192}$};
		\draw[thick,domain=-1/16:5/16] plot (\x,{((2-4*\x)/3)*(4/12+\x)});
		\draw[thick] (5/16,31/192) -- (5/8,23/96);
		\fill[black] (5/16,31/192) ellipse (0.0085 and 0.003) node[below left] {$\frac{31}{192}$};
		\fill[black] (5/8,23/96) ellipse (0.0085 and 0.003) node[above right] {$\frac{23}{96}$};
		\draw[thick,domain=5/8:1] plot (\x,{((2-2*\x)/3)*(4/12+\x)});

		% Max point
		\fill[red] (1/12, 25/108) ellipse (0.0085 and 0.003) node[below,black] {$\frac{25}{108}$};
	\end{tikzpicture}
	\begin{tikzpicture}[x=10cm, y=20cm, every node/.style={font=\small}]
		% Axes
		\draw[->] (0,0) -- (-0.55,0);
		...
		% Pseudo-heterogeneous profits
		\draw[gray,dashed,very thin,domain=-5/12:1/2] plot (\x,{((1-2*\x)/3)*(5/12+\x)}) node[above=20,right=-5,black] {$c = 2$};
		\draw[gray,dashed,very thin,domain=-5/12:1/4] plot (\x,{((1-4*\x)/3)*(5/12+\x)}) node[above=15,left=17,black] {$c = 4$};
		...
		\fill[black] (1/16,23/192) ellipse (0.005 and 0.0025) node[below left] {$\frac{23}{192}$};
		\fill[black] (1/8,13/96) ellipse (0.005 and 0.0025) node[above right] {$\frac{13}{96}$};
		\draw[thick,domain=1/8:1/2] plot (\x,{((1-2*\x)/3)*(5/12+\x)});

		% Max point
		\fill[red] (-1/12, 4/27) ellipse (0.008 and 0.004) node[above,black] {$\frac{4}{27}$};
	\end{tikzpicture}
	\caption{Profit functions for link operators $1$ and $2$ of Example \ref{example:a_step_profit_no_ne}.}
	\label{figure:alpha_step:profit_no_ne}
\end{figure}

It seems apparent, from the previous two examples, that the success of a candidate pricing Nash equilibrium to extend for a step function is heavily dependent on the function's definition, i.e. the sensitivity values as well as the amount of flow corresponding to them.
It would be tempting, at this point, to try and calculate exact conditions for the existence of such a Nash Equilibrium, given arbitrary game parameters.
For example, assume the pricing game in Example \ref{example:a_step_profit_ne} has the distribution function
\[
	\alpha(p) =
	\begin{cases}
		c & p \le p_0 \\
	...
	\end{cases}
\]
for $c < 4$ and $p_0 < 5/9$, with the last condition ensuring the sensitivity split for the equilibrium remains at $\as(t^*) = 4$.
...
We can then calculate $t_1$ such that $x(t_1, t_2^*) = p_0$, by solving the cost equality for $x = (p_0, 1 - p_0)$ and sensitivity $c$, and eventually get that link operator $1$ will not diverge from the equilibrium if
\[
	\frac{25}{108} \ge \frac{-3p_0^2 + p_0 (2 + c/3)}{c}
\]
Such a constrain can definitely provide insights into the numerical restrains between $c$ and $p_0$, however, as it also requires additional conditions to ensure the equilibrium won't still break due to higher profit at a different point, it suggests that a further analysis requires a more careful approach.
As, admittedly based on our own capacity, will not proceed with such an analysis.
We can still observe the following
\begin{itemize}
	\item larger differences in $\alpha_k$ values are more likely to break a pricing Nash Equilibrium
	\item larger flow intervals around $\as(t^*)$ are more likely to have a pricing Nash Equilibrium (the game becomes more homogeneous)
\end{itemize}
...
\begin{enumerate}
	...
	\item Calculate candidate pricing Nash Equilibrium at $t^* = \tfrac{t^{*\prime}}{\as(t^*)}$ and respective profits $\Pi_1(t^*), \Pi_2(t^*)$.
	...
	\begin{enumerate}[$4.1$]
		...
		\begin{enumerate}[$4.1.1$]
			\item If $B_{i, k}(t_{-i})$ is within the above ranges, then $\Pi_{i, k} = \Pi_i(B_{i, k}(t_{-i}), t_{-i})$.
			\item Otherwise, $\Pi_{i, k}$ is the max of the profits for tolls in the edges of the above ranges.
		\end{enumerate}
		\item If $\Pi_i < \Pi_{i, k}$, then $\Gm$ has no pricing Nash Equilibrium.
	\end{enumerate}
	\item If none of the above checks fail, return $t^*$ as the Nash Equilibrium for the pricing game in $\Gm$.
\end{enumerate}
