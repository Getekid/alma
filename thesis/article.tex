					...
					\begin{minipage}{7.5cm}
						\begin{flushleft}
							\normalsize {\it {\bf Examination committee:}\\
								Thanasis Lianeas,\\ External Collaborator, ALMA.\\
...
We show there is no Nash Equilibrium when there are many users with $0$ sensitivity, i.e. players not affected by tolls, forcing us to assume onward $\alpha(p) > 0$.
We then investigate cases of fixed distribution functions, where the user behaviour remains homogeneous (making them pseudo-heterogeneous games), through which we then describe how flow, tolls and equilibria behave as that fixed value changes.
Using all the above, we study step distribution functions, where, by limiting our scope to $2$ links, we prove a strong requirement for the value of a potential Nash Equilibrium in the profit game between the link operators, connecting it to the equilibrium of an equivalent pseudo-heterogeneous game.
We finally present and analyse an algorithm that calculates that equilibrium, if it exists, in time linear with regard to the step function's cardinality, and we finish with some discussion about potential future work.
\clearpage
...
In this chapter we will describe the model utilised throughout this thesis.
...
\end{equation*}
In that case, all links with $x_i > 0$ have equal effective costs, i.e. there exists some $K > 0$ such that for all those links $i$ it holds that $\ell_i(x_i) + t_i = K$.
...
We can easily calculate $x(0) = (2/3, 1/3)$ with equal cost $4/3$ in both links.
...
With players ordered in increasing sensitivity order, the less sensitive link $1$ users will have players $p_1 \in [0, 1/4]$ with $\alpha(p_1) \le \alpha(1/4) = 5/4$ and respectively link $2$ users $p_2 \in (1/4, 1]$ (player $x_i$ can be put arbitrarily in link $1$ or $2$) have $\alpha(p_2) \ge \alpha(1/4) = 5/4$.
...
This concept will be helpful because, in some cases, even though the parameters of a game might change, the players' relative utilities and respective strategies do not, making the two instances in practice identical.
...
In the analysis of Example \ref{example:simple_alpha}, we can see a first glimpse on how to tackle this issue, as we found out that the flow users on each link are organised around the player that views the two link costs as equal.
...
\end{itemize}
Remember that we have assumed that $t_1 > t_2 \stackrel{\text{\ref{lemma:latencies_tolls}}}{\Leftrightarrow} \ell_1(x_1(t)) < \ell_2(x_2(t))$.
...
\[t_i > t_j \Leftrightarrow t_i - t_j > 0 \LeftrightarrowArg{(\text{\ref{eq:split_property_cond_tolls_0}})} t_i^\prime - t_j^\prime > 0 \Leftrightarrow t_i^\prime > t_j^\prime\]
...
\begin{lemma}
	...
	Let $(N, \ell, \alpha)$ a heterogeneous parallel game and tolls $t, t^\prime$ such that $t_i \ne t_j, t_i^\prime \ne t_j^\prime$ for some links $i, j \in N$ and $t_k = t_k^\prime$ for all other $k \in N \setminus \{i, j\}$.
...
\begin{proof}
	...
	If $x_i(t) = x_i(t^\prime)$ and $x_j(t) = x_j(t^\prime)$, then looking at $\as^{(i, j)}$ we get
	...
	If $x_i(t) > x_i(t^\prime)$ and $x_j(t) < x_j(t^\prime)$ then there exists a player $p$ who is in link $i$ in the Nash equilibrium for $t$ and in link $j$ for the one for $t^\prime$, meaning that by applying (twice) Definition \ref{definition:alpha_flow_sets} we get
...
We apply this on Example \ref{example:simple_alpha} to solidify it.
...
\begin{proof}
	 ...
	 Therefore, $\as$ is upper bounded by its side limits as $t_i - t_j \rightarrow 0$, which means that for all $t$ in the domain of $\as$ we get
...
\begin{proof}
	...
	So if we take $\Gm_1$ where $p$ is in the lower split, then it holds that $p$ is not in $x_n^{(1)}$, since the link with the highest toll is always the upper split with regard to all others.
...
Special interest will then be on comparing such games, specifically those with the same setup but different fixed $\alpha$ functions, as Lemma \ref{lemma:a_fixed_c_relation} will reveal that, uniformly varying the player sensitivity maps toll values around the same flow.
...
\begin{proof}
	...
	Regardless of the heterogeneity of our users, since the above inequality holds, then for flow $x^{(2)}(t)$ in $\Gm_1$ it will also hold for any other flow $x^{(1)} = x^{(2)}$.
...
\end{proof}
Since a heterogeneous game with a fixed distribution function is, in practice, homogeneous with regard to the flow behaviour, we will also use, when relevant, the term \textit{pseudo-heterogeneous game} to describe such games more easily.
...
\begin{proof}
	\begin{enumerate}[$(i)$]
		...
		\[
			B_i^{(1)}(t_{-i}^{*(1)}) = \frac{c_1}{c_1} \cdot B_i^{(1)}\left(\frac{c_2}{c_2} t_{-i}^{*(1)}\right) \stackrel{\text{\ref{lemma:a_fixed_c_relation}}}{=} \frac{c_2}{c_1} \cdot B_i^{(2)}\left(\frac{c_1}{c_2}t_{-i}^{*(1)}\right)
		\]
		...
		\[
			x^{(1)}(t^{*(1)}) \stackrel{(i)}{=} x^{(1)}\left(\tfrac{c_2}{c_1}t^{*(2)}\right) \stackrel{\text{\ref{lemma:a_fixed_c_relation}}}{=} x^{(2)}(t^{*(2)})
		\]
	\end{enumerate}
\end{proof}
...
\begin{lemma}
	...
	\begin{enumerate}[$(i)$]
		\item $x^{(1)}(t) = x^{(2)}(t)$
		\item $\Pi_i^{(1)}(t) = \Pi_i^{(2)}(t)$ for all $i \in N$
	\end{enumerate}
\end{lemma}
...
We also know at which flows the behaviour of the network will change, which are exactly the flows where all players of each sensitivity type are using a single link, i.e. the flows $(4/5, 1/5), (1/2, 1/2), (1/5, 4/5)$, e.g. flow $(4/5, 1/5)$ has all players with $\alpha(p) = 1$ in link $2$ and all $\alpha(p) = 3, 4$ players in link $1$, which in turn means that such a flow can only have $\as$ value between $1$ and $3$.
...
Inversely, if all players of each sensitivity type are using a single link, then $\as$ can take any value between the highest and lowest $\alpha$ value in the lower and upper split respectively.
...
We complement this idea with the following Theorem, one of the main results of this chapter, from which it follows that, if that Nash Equilibrium exists, then it has to happen for some specific flow.
...
Unsurprisingly, there is a heavy dependence on the definition of the distribution function, which we illustrate in the following two examples.
...
\begin{proof}
	...
	We can calculate the latter equilibrium $t^{*\prime}$ by solving the system of equations in \ref{eq:homogeneous_x_i} and \ref{eq:homogeneous_pricing_ne_i}, from which we get $t^{*\prime} = \left(\tfrac53, \tfrac43\right)$ with $x^\prime(t^{*\prime}) = \left(\tfrac59, \tfrac49\right)$.
	...
	\[
		t^* = \frac14 t^{*\prime} = \left(\frac{5}{12}, \frac{4}{12}\right) \qquad x(t^*) = x^\prime(t^{*\prime}) = \left(\frac59, \frac49\right)
	\]
	We're only left with checking whether $t^*$ satisfies the definition of a Nash Equilibrium, i.e. whether each link operator has a max profit toll in response to the opposing one.
	We will therefore need to calculate the profit functions for each link, for which, thanks to \ref{lemma:heterogenous_flow_equiv_fixed}$(ii)$, we can use the respective pseudo-heterogeneous profit functions for $c = 2, 3$.
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Profit functions for link operators $1$ and $2$ of Example \ref{example:a_step_profit_ne} when opposed against the candidate Nash Equilibrium $t^* = \left(\tfrac{5}{12}, \tfrac{4}{12}\right)$. Red dots indicate the max profit at $t^*$. Black dots indicate tolls when relevant intervals start or end for the respective pseudo-heterogeneous profits (dashed lines) they correspond.}
	\label{figure:alpha_step:profit_ne}
\end{figure}

\begin{example}
	\label{example:a_step_profit_no_ne}
	Let $\Gm = ([2], \ell, \alpha)$ a heterogeneous pricing game with latency functions $\ell_1(x) = 2x, \ell_2(x) = x + 1$ and distribution function $\alpha$ such that
	\begin{align*}
		\alpha(p) =
		\begin{cases}
...
\begin{proof}
	Similarly to Example \ref{example:a_step_profit_ne}, we have from Theorem \ref{theorem:a_step_ne_equiv_fixed} that $t^* = \left(\tfrac{5}{12}, \tfrac{4}{12}\right)$ is the only candidate pricing Nash Equilibrium.
	We calculate again the profit functions, which we present in Figure \ref{figure:alpha_step:profit_no_ne}, and where we can see that
	\[
		B_1\left(\frac{4}{12}\right) = \frac{23}{24} \ne \frac{5}{12} \qquad B_2\left(\frac{5}{12}\right) = \frac{4}{12}
	\]
	which makes $t^*$ ineligible for an equilibrium.
	Since $t^*$ is the only possible candidate (Theorem \ref{theorem:a_step_ne_equiv_fixed}), it follows that $\Gm$ has no Nash equilibrium for its pricing game.
\end{proof}
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Profit functions for link operators $1$ and $2$ of Example \ref{example:a_step_profit_no_ne} when opposed against the candidate Nash Equilibrium $t^* = \left(\tfrac{5}{12}, \tfrac{4}{12}\right)$. Red dots indicate the max profit at $t^*$. Black dots indicate tolls when relevant intervals start or end for the respective pseudo-heterogeneous profits (dashed lines) they correspond.}
...
\begin{enumerate}
	...
	\begin{enumerate}[$4.1$]
		\item Consider $\Gm_k = ([2], \ell, \alpha_k)$ and calculate $B_{i, k}(t_{-i}^*)$ as well as the tolls $t_i$ for which $\as(t_i, t_{-i}^*) = \alpha_k$, resulting in two toll ranges for $t_i$.
		...
		\begin{enumerate}[$4.2.1$]
			...
			\item Otherwise, $\Pi_{i, k}$ is the highest profit value among the ($4$) tolls in the edges of the two $t_i$ ranges.
		\end{enumerate}
...
\begin{theorem}
	Let $\Gm = ([2], \ell, \alpha)$ a heterogeneous game with latency functions $\ell = (\ell_i(x))_{i \in [2]} = (a_ix + b_i)_{i \in [2]}$ and $\alpha$ a step distribution function with values $(\alpha_k)_{k \in [m]}$.
...
\begin{proof}
	...
	Otherwise, since $\Pi_i(t)$ is concave from the analysis of Harkes et al. \cite{Harks_2019}, not taking a max value at a range follows that profit is either non-decreasing or non-increasing in that range, resulting in its local maximum to be achieved at one of its edge values.
	...
	Theorem \ref{theorem:alpha_upper_irrelevant} also shows us there might be sensitivity values that are irrelevant to the game.
	The algorithm will ignore all those cases, however, as the \texttt{TollDiff} function will return $t_i < 0$, resulting in $\Pi_i < 0$, which will be skipped later when looking for max profit values.

	Finally, regarding the complexity of Algorithm \ref{algorithm:step_ne_pricing}, we can easily see that it mostly consists of base operations, with only a \texttt{for} loop taking at most $m - 1$ iterations, resulting in a time complexity of $\mathcal{O}(m)$.
\end{proof}
...
The main issue with that potential extension is that, even for $3$-link parallel networks, the notion of a single user viewing all link costs equally is lost, as different sensitivity splits between any two pairs of links will make it so that no sensitivity value can view their cost all as equal.
...
Finally, another way to characterise the pricing game for some $n$-link networks, would be proving it never exists for $n \ge 3$ under some conditions, a possibility which the author have suspected.
The main motivation for such a suspicion is that, even for $n = 3$, with the condition of having different sensitivity splits between the two consecutive pairs of links (with regard to a toll ordering on links), the respective sub-games of those pairs will likely display a behaviour similar to $2$-link networks, thus each converging to their own pricing equilibrium and never reaching one overall.
...
\begin{proof}
	...
	For affine latency functions, we can calculate the best response functions $B_i(t_{-i})$ for each link operator $i$, by solving first order conditions on $\Pi_i(t_i, t_{-i})$ for $t_i$.
	\begin{equation}
...
\begin{lemma}
	...
	Let $\Gm_1 = ([2], \ell, c_1)$ and $\Gm_2 = ([2], \ell, c_2)$ two pseudo-heterogeneous pricing games with same latency functions $\ell$ and distribution functions $\alpha^{(1)}(p) = c_1$ and $\alpha^{(2)}(p) = c_2 > c_1 > 0$ respectively.
...
\begin{proof}
	...
	\begin{enumerate}[$(i)$]
		...
		Then for $t_{-i} = t_{-i_0}^{(2)}$, the best response for link operator $i$ in $\Gm_2$, by definition, is to play the same toll as in the other link, therefore they get max profit at $x_i^{(2)}(0) t_{-i_0}^{(2)}$.
		In $\Gm_1$ now, since $t_{-i_0}^{(2)} < t_{-i_0}^{(1)}$, Lemma \ref{lemma:tolls_diff} gives us
		\[
			B_i^{(1)}(t_{-i_0}^{(2)}) - t_{-i_0}^{(2)} > B_i^{(1)}(t_{-i_0}^{(1)}) - t_{-i_0}^{(1)} \Rightarrow B_i^{(1)}(t_{-i_0}^{(2)}) > t_{-i_0}^{(2)}
		\]
...
As such, $\forall k \in [m]$ we denote as $P_k$ the closed set of all players $p$ in $[0, 1]$ for whom $\alpha(p) = \alpha_k$.
Obviously $\cup_{k=1}^m P_k = [0, 1]$ and from the assumption that $\alpha$ is non-decreasing we also get $P_k$ convex $\forall k \in [m]$.
...
\begin{definition}
	Let $\Gm_1 = ([2], \ell, \alpha)$ a heterogeneous game where $\alpha$ is a step function and $(P_k)_{k \in [m]}$ the partition of $[0, 1]$ that is defined by the domain of $\alpha$.
	Then for link operator $i$ the definition of $\Pi_i$ is extended for an interval $P_k$ as
	\[
...
So as $|t_1 - t_2| \rightarrow +\infty$ in each case, $\as$ becomes respectively
\[
...
Lemma \ref{lemma:split_basic}$(iii)$, combined with the fact that $\alpha$ is continuous, gives us that $\as(t) = \alpha(x_i(t))$ for $x_i$ the lower split flow.
...
\subparagraph{Gathering all the above} we see that link operator $1$, when searching for the best response toll, will compare all profits that tolls provide in each case, so we calculate them.
\[
	\Pi_1^>(t_2) =
	\begin{cases}
