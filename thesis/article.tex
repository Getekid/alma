...
We describe this by adding a money-sensitivity weight $\alpha(p)$ on the toll costs experienced by the player, making them see the cost of each edge as $c_i^p(x(t)) = \ell_i(x_i) + \alpha(p) \cdot t_i$ and then seek the shortest link among them.
...
Finally, we will assume that $\alpha(0) > 0$, since, as we'll see in more detail in Example \ref{example:a_0_0}, allowing many players close to $0$ breaks the pricing game.

With the above we can again define a Nash equilibrium for the game if $\forall i \in N$ and $\forall p \in [0, 1]$ it holds that
\[c_{f(p)}^p(x(t)) \leq c_i^p(x(t))\]
...
Focusing on the model that will be used in this thesis, simplify our network by only having two links but consider that the flow is acting in a heterogeneous way.
The model can thus be defined as the tuple $(\ell, \alpha)$, where $\ell = (\ell_1, \ell_2)$ the latency functions and $\alpha$ the distribution function.
[TODO: Add figure]
