...
\begin{proof}
	...
	\begin{enumerate}[(i)]
		...
		Otherwise $x_1(t) > x_1(t^\prime) \Rightarrow \exists \epsilon > 0: x_1(t) = x_1(t^\prime) + \epsilon$.
		...
		%\end{equation}
		Finally, by carefully applying property (ii) we get
		\[\as(t^\prime) \le \alpha(x_1(t^\prime) + \epsilon) = \alpha(x_1(t)) \le \as(t)\]
		
		All steps can be followed backwards as well, therefore the opposite also holds.
		
		For the second case, we see that the toll difference decreases, and following respectively the same arguments we can show that $\as(t) \le \as(t^\prime)$.
