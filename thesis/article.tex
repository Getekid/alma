...
A \textit{flow} is a (stochastic) vector $x = (x_i)_{i \in N}$ where $x_i$ is the total flow on link $i$.
...
We describe this by adding a money-sensitivity weight $\alpha(p)$ on the toll costs experienced by the player, making them see the cost of each edge as $c_i^p(x, t) = \ell_i(x_i) + \alpha(p) t_i$ and then seek the shortest link among them.
...
Also we will assume that $\alpha(0) > 0$, since, as we'll see in more detail in Section ??, allowing many players close to $0$ breaks the pricing game.

\section*{Pricing Games}

We introduce $n$ link owners which can assign a toll $t_i$ on link $i$ and profit from it.

\section*{2-link Pricing Games (our model)}
